...
var SettingsTab = class extends import_obsidian11.PluginSettingTab {
  ...
  display() {
    ...
    const nameSetting = new import_obsidian11.Setting(containerEl).setName("Folder note name").setDesc("{{folder_name}} will be replaced with the name of the folder").addText((text) => text.setValue(this.plugin.settings.newFolderNoteName).onChange(async (value) => {
      if (value.trim() === "") {
        return;
      }
    ...
    new import_obsidian11.Setting(containerEl).setName("Folder note type").setDesc("Choose the file type for creating new folder notes (markdown or canvas) old folder notes will not be changed and you can also create canvas/markdown files and change the name manually to the custom folder note name or to the name of the folder.").addDropdown((dropdown) => dropdown.addOption(".md", "markdown").addOption(".canvas", "canvas").setValue(this.plugin.settings.folderNoteType).onChange(async (value) => {
      this.plugin.settings.folderNoteType = value;
      await this.plugin.saveSettings();
    }));
    const storageLocation = new import_obsidian11.Setting(containerEl).setName("Storage location").setDesc("Choose where to store the folder notes").addDropdown((dropdown) => dropdown.addOption("insideFolder", "Inside the folder").addOption("parentFolder", "In the parent folder").setValue(this.plugin.settings.storageLocation).onChange(async (value) => {
    ...
    const switchLocation = new import_obsidian11.Setting(containerEl).setName("Switch to new storage location").setDesc("Move all folder notes to the new storage location").addButton((button) => button.setButtonText("Switch").setCta().onClick(async () => {
    ...
    if (this.plugin.settings.storageLocation === "parentFolder") {
      new import_obsidian11.Setting(containerEl).setName("Delete folder notes when deleting the folder").setDesc("Delete the folder note when deleting the folder").addToggle((toggle) => toggle.setValue(this.plugin.settings.syncDelete).onChange(async (value) => {
        this.plugin.settings.syncDelete = value;
        await this.plugin.saveSettings();
      }));
      new import_obsidian11.Setting(containerEl).setName("Move folder notes when moving the folder").setDesc("Move the folder note when moving the folder").addToggle((toggle) => toggle.setValue(this.plugin.settings.syncMove).onChange(async (value) => {
        this.plugin.settings.syncMove = value;
        await this.plugin.saveSettings();
      }));
    }
    const disableSetting = new import_obsidian11.Setting(containerEl);
    ...
    new import_obsidian11.Setting(containerEl).setName("Only open folder notes through the name").setDesc("Only open folder notes in the file explorer by clicking on the folder name").addToggle((toggle) => toggle.setValue(this.plugin.settings.allowWhitespaceCollapsing).onChange(async (value) => {
      if (!value) {
    ...
    new import_obsidian11.Setting(containerEl).setName("Hide folder note").setDesc("Hide the folder note in the file explorer").addToggle((toggle) => toggle.setValue(this.plugin.settings.hideFolderNote).onChange(async (value) => {
    ...
    new import_obsidian11.Setting(containerEl).setName("Sync folder name").setDesc("Automatically rename the folder note when the folder name is changed").addToggle((toggle) => toggle.setValue(this.plugin.settings.syncFolderName).onChange(async (value) => {
      this.plugin.settings.syncFolderName = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (import_obsidian11.Platform.isDesktop) {
      const setting3 = new import_obsidian11.Setting(containerEl);
    ...
    new import_obsidian11.Setting(containerEl).setName("Automatically create folder notes").setDesc("Automatically create a folder note when a new folder is created").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoCreate).onChange(async (value) => {
      this.plugin.settings.autoCreate = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian11.Setting(containerEl).setName("Use submenus").setDesc("Use submenus for file/folder commands").addToggle((toggle) => toggle.setValue(this.plugin.settings.useSubmenus).onChange(async (value) => {
      this.plugin.settings.useSubmenus = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (import_obsidian11.Platform.isDesktopApp) {
      new import_obsidian11.Setting(containerEl).setName("Key for creating folder note").setDesc("The key combination to create a folder note").addDropdown((dropdown) => {
    ...
    new import_obsidian11.Setting(containerEl).setName("Add underline to folders with folder notes").setDesc("Add an underline to folders that have a folder note in the file explorer").addToggle((toggle) => toggle.setValue(this.plugin.settings.underlineFolder).onChange(async (value) => {
    ...
    new import_obsidian11.Setting(containerEl).setName("Open folder note through path").setDesc("Open a folder note when clicking on a folder name in the path if it is a folder note").addToggle((toggle) => toggle.setValue(this.plugin.settings.openFolderNoteOnClickInPath).onChange(async (value) => {
      this.plugin.settings.openFolderNoteOnClickInPath = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (this.plugin.settings.openFolderNoteOnClickInPath) {
      new import_obsidian11.Setting(containerEl).setName("Underline folders in the path").setDesc("Add an underline to folders that have a folder note in the path above a note").addToggle((toggle) => toggle.setValue(this.plugin.settings.underlineFolderInPath).onChange(async (value) => {
    ...
    const manageExcluded = new import_obsidian11.Setting(containerEl).setHeading().setName("Manage excluded folders");
    ...
    new import_obsidian11.Setting(containerEl).setName("Add excluded folder").addButton((cb) => {
      cb.setIcon("plus");
  ...
  updateFolderNotes(oldTemplate, newTemplate) {
    ...
    this.plugin.saveSettings();
    new import_obsidian11.Notice("Starting to update folder notes...");
    this.app.vault.getFiles().forEach((file) => {
      if (file instanceof import_obsidian11.TFile) {
        ...
        if (!(folder instanceof import_obsidian11.TFolder) || folderName !== (folder == null ? void 0 : folder.name)) {
          if (folderPath.trim() === "") {
    ...
    new import_obsidian11.Notice("Finished updating folder notes");
  }
  switchStorageLocation(oldMethod) {
    new import_obsidian11.Notice("Starting to switch storage location...");
    ...
    new import_obsidian11.Notice("Finished switching storage location");
  }
};
  ...
  registerCommands() {
        ...
        item.setTitle("Create folder note").setIcon("edit").onClick(() => {
          ...
          for (const char of blacklist) {
            if (text.includes(char)) {
              new import_obsidian12.Notice('File name cannot contain any of the following characters: * " \\ / < > : | ?');
              return;
            }
          }
          if (text.endsWith(".")) {
            new import_obsidian12.Notice("File name cannot end with a dot");
            return;
          }
          let folder;
          const folderPath = this.plugin.getFolderPathFromString(file.path);
          if (folderPath === "") {
            ...
            if (folder instanceof import_obsidian12.TFolder) {
              return new import_obsidian12.Notice("Folder note already exists");
            } else {
            ...
            if (this.plugin.settings.storageLocation === "parentFolder") {
              if (this.app.vault.getAbstractFileByPath(folderPath + "/" + text + this.plugin.settings.folderNoteType)) {
                return new import_obsidian12.Notice("File already exists");
              }
            }
    ...
    this.plugin.registerEvent(this.app.workspace.on("file-menu", (menu, file) => {
      ...
      if (file instanceof import_obsidian12.TFile) {
        ...
        } else if (file.parent instanceof import_obsidian12.TFolder) {
          folder = file.parent;
        }
      }
      menu.addItem((item) => {
        if (import_obsidian12.Platform.isDesktop && !import_obsidian12.Platform.isTablet && this.plugin.settings.useSubmenus) {
          item.setTitle("Folder Note Commands").setIcon("folder-edit");
        }
        let subMenu;
        if (!import_obsidian12.Platform.isDesktopApp || !import_obsidian12.Platform.isDesktop || import_obsidian12.Platform.isTablet || !this.plugin.settings.useSubmenus) {
          subMenu = menu;
        ...
        if (this.plugin.settings.excludeFolders.find((folder2) => folder2.path === file.path)) {
          subMenu.addItem((item2) => {
            item2.setTitle("Remove folder from excluded folders").setIcon("trash").onClick(() => {
              ...
              this.plugin.saveSettings();
              new import_obsidian12.Notice("Successfully removed folder from excluded folders");
            });
          });
          return;
        }
        subMenu.addItem((item2) => {
          item2.setTitle("Exclude folder from folder notes").setIcon("x-circle").onClick(() => {
            ...
            this.plugin.saveSettings();
            new import_obsidian12.Notice("Successfully excluded folder from folder notes");
          });
        });
        if (!(file instanceof import_obsidian12.TFolder))
          return;
...
async function handleViewHeaderClick(event, plugin) {
  ...
  } else if (event.altKey || import_obsidian13.Keymap.isModEvent(event) === "tab") {
    if (plugin.settings.altKey && event.altKey || plugin.settings.ctrlKey && import_obsidian13.Keymap.isModEvent(event) === "tab") {
...
function handleFileRename(file, oldPath, plugin) {
  ...
  if (folderName === (newFolder == null ? void 0 : newFolder.name) && folderNote) {
    new import_obsidian14.Notice("Folder with same name already exists!");
    let excludedFolderExisted = true;
    let disabledSync = false;
    if (!excludedFolder) {
      excludedFolderExisted = false;
...
async function renameFolderOnFileRename(file, oldPath, oldFolder, plugin) {
  ...
  if (plugin.app.vault.getAbstractFileByPath(newFolderPath) || plugin.app.vault.getAbstractFileByPath(newFolderName || "")) {
    ...
    return new import_obsidian14.Notice("A folder with the same name already exists");
  }
...
function createOverview(plugin, source, el, ctx) {
  let yaml = (0, import_obsidian15.parseYaml)(source);
  if (!yaml) {
    yaml = {
  ...
  let sourceFolder;
  if (sourceFile.parent instanceof import_obsidian15.TFolder) {
...
function sortFiles(files, yaml, plugin) {
  ...
  return files.sort((a, b) => {
    if (a instanceof import_obsidian15.TFolder && !(b instanceof import_obsidian15.TFolder)) {
      return -1;
    }
    if (!(a instanceof import_obsidian15.TFolder) && b instanceof import_obsidian15.TFolder) {
      return 1;
    }
    if (!(a instanceof import_obsidian15.TFile) || !(b instanceof import_obsidian15.TFile)) {
      return -1;
    }
...
var FolderOverviewSettings = class extends import_obsidian16.Modal {
  ...
  display() {
    ...
    new import_obsidian16.Setting(contentEl).setName("Disable the title").setDesc("Choose if the title should be shown").addToggle((toggle) => toggle.setValue(this.yaml.disableTitle || false).onChange(async (value) => {
      this.yaml.disableTitle = value;
      this.display();
      await this.updateYaml();
    }));
    if (!this.yaml.disableTitle) {
      new import_obsidian16.Setting(contentEl).setName("Title").setDesc("Choose the title of the folder overview").addText((text) => {
        var _a2;
    ...
    const setting = new import_obsidian16.Setting(contentEl);
    ...
    if (disableFileTag) {
      new import_obsidian16.Setting(contentEl).setName("Disable file tag").setDesc("Choose if the file tag should be shown after the file name").addToggle((toggle) => {
    ...
    new import_obsidian16.Setting(contentEl).setName("File depth").setDesc("File & folder = +1 depth").addSlider((slider) => {
      var _a2;
    ...
    new import_obsidian16.Setting(contentEl).setName("Overview style").setDesc("Choose the style of the overview (grid style soon)").addDropdown((dropdown) => {
      var _a2;
    ...
    new import_obsidian16.Setting(contentEl).setName("Sort files by").setDesc("Choose how the files should be sorted").addDropdown((dropdown) => {
      var _a2;
    ...
    new import_obsidian16.Setting(contentEl).setName("Show folder names of empty folders").setDesc("That includes subfolders of the current folder when you are on file depth 1").addToggle((toggle) => {
    ...
    if (this.yaml.showEmptyFolders) {
      new import_obsidian16.Setting(contentEl).setName("Only show first empty subfolders of current folder").addToggle((toggle) => {
  ...
  async updateYaml() {
    ...
    if (!(file instanceof import_obsidian16.TFile))
      return;
    let stringYaml = (0, import_obsidian16.stringifyYaml)(this.yaml);
...
import_obsidian17.Setting.prototype.createList = createList;

// src/main.ts
var FolderNotesPlugin = class extends import_obsidian18.Plugin {
  async onload() {
      ...
      const observer = new MutationObserver(() => {
        ...
        if (editButton) {
          editButton.addEventListener("click", (e) => {
            ...
            e.preventDefault();
            e.stopPropagation();
            new FolderOverviewSettings(this.app, this, (0, import_obsidian18.parseYaml)(source), ctx, el).open();
          }, { capture: true });
        }
      });
      observer.observe(el, {
